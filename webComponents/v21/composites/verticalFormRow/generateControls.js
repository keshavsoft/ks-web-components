...
export function generateControls(element) {
    ...
    if (labelText) {
        ...
        label.setAttribute("ks-class-name", "font-semibold text-gray-700");
    ...
    if (buttonText) {
        const buttonControl = document.createElement("ks-web-component");
        buttonControl.setAttribute("ks-control-type", "button");
        buttonControl.setAttribute("ks-text", buttonText);
        buttonControl.setAttribute("ks-theme", element.getAttribute("ks-button-theme") || "default");
        
        if (themeConfig.buttonWrapper) {
            // Note: button wrappers in vertical forms usually require a div because 
            // of flex alignment. ks-web-component can receive these grid/flex classes directly now!
            buttonControl.className = themeConfig.buttonWrapper;
        }
